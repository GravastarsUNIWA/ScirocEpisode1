...
class SpawnOrder(smach.State):
    def __init__(self, msg):
        smach.State.__init__(self, outcomes=['done'])
        self.msg = msg
        self.get_name_of_table_order_sub = rospy.Subscriber(
            "/table_status/table_to_order", String, self.get_name_of_table_order_callback)
    ...
    def execute(self, userdata):
        ...
        os.system('rosservice call /sciroc_object_manager/get_three_ordered_items %s %s %s' %
...
class SpawnYolo(smach.State):
    ...
    def execute(self, userdata):
        ...
        cli_args = ['ros_yolov5', 'ros_yolov5_plus_ultra.launch',
                    'weights_path:=/home/paris/tiago_public_ws/src/ros_yolov5/weights/plus_ultra.pt']
...
class ConfirmOrder(smach.State):
    ...
    def __init__(self, msg):
        ...
        self.get_name_of_table_order_sub = rospy.Subscriber(
            "/table_status/table_to_order", String, self.get_name_of_table_order_callback)
        self.order_sub = rospy.Subscriber(
            "/table_status/all_table_status", String, self.order_callback)
    ...
    def order_callback(self, msg):
        if self.order_table_name:
            encoded_items = msg.data
            decoded_items = json.loads(encoded_items)
            self.items_on_order = str(
                decoded_items[self.order_table_name]['Order'])

    def execute(self, userdata):
        topic = '/object_detection/objects'
        msg = rospy.wait_for_message(topic, String)
        items_on_paso = json.loads(msg.data)
        print("peos2 ", items_on_paso, type(items_on_paso), len(items_on_paso))

        if len(items_on_paso) > 2:
            items_on_paso = [items_on_paso[0][-1],
                             items_on_paso[1][-1], items_on_paso[2][-1]]
        ...
        elif len(items_on_paso) <= 2:
            print("DID NOT SEE 3 ITEMS\n")
            return 'correct'
...
class CorrectOrder(smach.State):
    ...
    def execute(self, userdata):
        ...
        self.wrong_item = collections.Counter(
            items_on_paso) - collections.Counter(self.items_on_order)
        self.wrong_item = self.wrong_item.keys()
        self.wrong_item_num = (collections.Counter(
            items_on_paso) - collections.Counter(self.items_on_order)).items()[0][1]
        self.correct_item = collections.Counter(
            self.items_on_order) - collections.Counter(items_on_paso)
        self.correct_item = self.correct_item.keys()
        self.correct_item_num = (collections.Counter(
            self.items_on_order) - collections.Counter(items_on_paso)).items()[0][1]
        os.system('rosservice call /sciroc_object_manager/change_the_item %s %s' %
                  (self.wrong_item[0], self.correct_item[0]))
        return 'done'
...
class ServeItems(smach.State):
    ...
    def serving_callback(self, msg):
        ...
        self.decoded_all_table_status = json.loads(
            self.encoded_all_table_status)
